...
async fn propose_trade(ctx: &CommandContext<'_>) -> anyhow::Result<()> {
...

    {
        ...
        cooldowns.insert(sender_uuid, Instant::now() + Duration::from_millis(ctx.runtime.player_economy.trade_propose_cooldown_ms));
    }
    ...
    match ctx.state.api.tradebot_reject_trade(trade_id).await {
        Ok(()) => {
            ...
            if is_recipient {
                ...
                cooldowns.insert(initiator_id, Instant::now() + Duration::from_millis(ctx.runtime.player_economy.trade_reject_penalty_ms));
            }
        }

...
impl GameState {
    ...
    fn is_betting_round_complete(&self) -> bool {
        ...
        if self.player_stack == 0 || self.bot_stack == 0 {
            let allin_bet = if self.player_stack == 0 { self.player_bet } else { self.bot_bet };
            let other_bet = if self.player_stack == 0 { self.bot_bet } else { self.player_bet };
    ...
    pub fn amount_to_call(&self, player: Player) -> u32 {
        ...
        if max > current { max - current } else { 0 }
    }

...
async fn load_markets(ctx: &CommandContext<'_>, category: &str) -> Result<Vec<KalshiMarket>, FetchErr> {
    let cached = {
        ...
        if age < ctx.runtime.casino.kalshi_cache_ttl_ms / 1000 && cache.category == category && !cache.markets.is_empty() {
...
async fn place_bet(ctx: &CommandContext<'_>) -> anyhow::Result<()> {
    ...
    let limit = ctx.bet_limit("kalshi", MIN_BET, None);
    if stake < limit.min {
...
pub async fn settle_task(
    ...
    let (max_poll_ms, poll_interval_ms) = {
        ...
        (runtime.casino.kalshi_max_poll_ms, runtime.casino.kalshi_poll_interval_ms)
    };

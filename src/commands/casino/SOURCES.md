...
|--------|-----------|-----------|
| `baccarat.rs` | _Let's Go Gambling!_ — `BaccaratScreenHandler.java` by BobR0ssiter. Card scoring (`bac_value` / `bac_score`), natural detection, simplified drawing rules (both sides draw on ≤5), and payout ratios (Player 2×, Banker 1.95×, Tie 8×) adapted from source. No session state (instant resolve); chip integration, command flow original. Card draws now come from `shoe.rs` (see below) instead of independent per-card RNG. | MIT — © BobR0ssiter. Modrinth: https://modrinth.com/mod/QCz7p8r1 |
| `slots.rs` | _slot-machine-gen_ by Marc S. Brooks. Weighted symbol selection algorithm (`selectRandSymbol` iterate-subtract pattern) adapted from source; that library has no payout/scoring logic of its own (display/animation only). Pays on 5 lines (3 rows + 2 diagonals) from 3 reels — no public PAR sheet exists for a real machine of this exact shape (checked 2026-08-03). Per-symbol payout multipliers are fitted (log-log power regression) from Lucky Larry's Lobstermania's real, FOIA-obtained 96.2%-RTP reel/pay data — Harrigan, K. A., & Dixon, M. (2009). "PAR Sheets, probabilities, and slot machine play: Implications for problem and non-problem gambling." *Journal of Gambling Issues*, 23. `REFERENCE_MATERIAL/DOCS/Harrigan-Dixon-2009-PAR-Sheets-Probabilities-Slot-Machine-Play.pdf`, Table 3 — then rescaled to this project's mandatory 97% RTP (3% house rake), verified via exhaustive brute-force over all reel-stop combinations. Original bug (2026-08-03 audit): the prior invented multiplier table paid 344% RTP, never caught since the 5-line sum was never verified against the single-line RTP the comment claimed. Symbol set is original. | MIT — © 2020-2025 Marc S. Brooks; PAR data © Harrigan & Dixon 2009, cited under academic fair use |
...
| `blackjack.rs` | _Let's Go Gambling!_ — `BlackjackScreenHandler.java` by BobR0ssiter. Card scoring, soft-ace logic, dealer-to-17, natural 3:2, double-down adapted from source. Split and side bets (Perfect Pairs / 21+3) not ported (chat UX). Card draws now come from `shoe.rs` (see below) instead of independent per-card RNG. | MIT — © BobR0ssiter. Modrinth: https://modrinth.com/mod/QCz7p8r1 |
| `shoe.rs` | Shuffle-and-deal mechanism (`Vec<Card>` built once, `rand::seq::SliceRandom::shuffle`, index-based `deal()` with depletion) adapted from _terminal-poker_'s `game/deck.rs` (already used directly by `poker/`, see below) — reused here as a rank-only, config-driven multi-deck shoe (`casino_deck_count`, default 6). One shared shoe per game (like a real table, not a personal deck), dealt to every `blackjack.rs`/`baccarat.rs` player. Persists across hands (1hr lifetime or exhaustion triggers an announced reshuffle), unlike poker/hilo which build a fresh single deck per hand/round. | MIT — © 2025 ashxudev. Repo: https://github.com/ashxudev/terminal-poker |
...
| `poker/` | _terminal-poker_ — Rust NLHE engine by ashxudev (2025). Extracted: `game/deck.rs`, `game/hand.rs`, `game/state.rs`, `game/actions.rs`, `bot/rule_based.rs`, `bot/draws.rs`, `bot/preflop.rs`. TUI, stats, and main event loop not used. `crate::game::` imports rewritten to relative paths. `game/deck.rs`'s shuffle-and-deal mechanism separately reused (not the Card/Suit types) in `shoe.rs` for blackjack/baccarat's multi-deck shoe — see below. | MIT — © 2025 ashxudev. Repo: https://github.com/ashxudev/terminal-poker |

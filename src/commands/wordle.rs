        ...
        match arg.as_str() {
            "" => {
                if has_active_game(ctx.state, ctx.sender) {
                    ...
                    ctx.whisper("Usage: !wordle <chips> [hard] | guess: !wordle <word> | !wordle board | !wordle quit");
                }
            }
            "board" => show_board(&ctx),
            "quit" | "forfeit" => quit_game(&ctx).await?,
            "help" => {
                ctx.whisper("!wordle <chips> [hard] — start | !wordle <word> — guess | !wordle board — show board | !wordle quit — forfeit");
            }
            ...
            _ => {
                let chips: i64 = match arg.parse() {
                    Ok(n) => n,
                    Err(_) => {
                        ctx.whisper("Unknown command. Try !wordle help.");
                        return Ok(());
                    }
                };
...
async fn start_game(ctx: &CommandContext<'_>, stake: i64, hard_mode: bool) -> anyhow::Result<()> {
    ...
    if has_active_game(ctx.state, ctx.sender) {
        ctx.whisper("Already have an active game. !wordle board to see it, !wordle quit to forfeit.");
        return Ok(());
    }
...
async fn submit_guess(ctx: &CommandContext<'_>, word: &str) -> anyhow::Result<()> {
    ...
    let outcome = {
        ...
        let session = match games.get_mut(ctx.sender) {
            ...
            None => {
                drop(games);
                return {
                    ctx.whisper("No active game. Start one with !wordle <chips>.");
                    Ok(())
                };
            }
        };
    ...
    match outcome {
        GuessOutcome::NotInWordList => {
            ctx.whisper(format!("'{}' isn't in the word list.", word.to_uppercase()));
        }
        GuessOutcome::HardModeViolation(i) => {
            ctx.whisper(format!("Hard mode: position {} must stay fixed.", i + 1));
        }
        GuessOutcome::Continue { board, guesses_used } => {
            for line in &board { ctx.whisper(line); }
            ctx.whisper(format!("Guess {}/{}.", guesses_used, MAX_GUESSES));
        }
        GuessOutcome::Win { board, guesses_used, stake } => {
